# Company Report - Sectors Financial API
...
```
curl --request GET \
  --url https://api.sectors.app/v2/company/report/{symbol}/ \
...
fetch('https://api.sectors.app/v2/company/report/{symbol}/', options)
  .then(res => res.json())
...
curl_setopt_array($curl, [
  CURLOPT_URL => "https://api.sectors.app/v2/company/report/{symbol}/",
...
```
HttpResponse<String> response = Unirest.get("https://api.sectors.app/v2/company/report/{symbol}/")
  .header("Authorization", "<api-key>")
  .asString();
```
...
url = URI("https://api.sectors.app/v2/company/report/{symbol}/")
...
IDX symbol: 4 letters, optionally followed by `.jk` (case-insensitive). E.g. `BREN`, `BBCA`, `TLKM`.
...
Costs 1 API credit per requested section. Default behavior (all 8 sections) consumes 8 credits.
...
IDX symbol symbol. E.g. `BREN`, `BBCA`.
...
Comma-separated list of sections to include. Default to all.
...
Listing board, industry, sector, market cap, price, ESG, tags.
...
PE/PB/PS, intrinsic value, forecasts, historical valuation.
...
Analyst value/growth forecasts and rating breakdown.
...
EPS, historical financials & ratios, YoY growth.
...
Historical/upcoming dividends, yield, payout ratio.
...
Major shareholders, transactions, whale investors.
